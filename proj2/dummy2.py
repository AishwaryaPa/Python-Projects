...
def letter_counter(first_letter, two_d_list, width, length):
    counter = 0
    for row in range(width):
        for column in range(length):
            if two_d_list[row][column] == first_letter:
                counter = counter + 1
...
def main():
    ...
    for line in puzzle_open:
        ...
        # get rid of spaces between characters in the puzzle 
